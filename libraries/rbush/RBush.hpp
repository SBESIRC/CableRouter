...
namespace rbush
{
   ...
   // calculate node's bbox from bboxes of its children
    ...
    static void quickselect(std::vector<TreeNode<T> *>array, int left, int right, int k, std::function<double(const TreeNode<T> *, const TreeNode<T> *)> compare)
    {
      ...
      while (right > left)
      {
        ...
        // grouping the elements between left and right around t
        auto t = array[k];
        int i = left;
        int j = right;
    ...
    TreeNode<T> *_build(std::vector<TreeNode<T> *>& items, int left, int right, int height)
    {
      ...
      if (!height)
      {
        ...
        height = std::ceil(std::log((double)N) / std::log((double)M));

        // target number of root entries to maximize storage utilization
        M = std::ceil(N / std::pow((double)M, (double)height - 1));
      }
      ...
      int N2 = std::ceil((double)N / M);
      int N1 = N2 * std::ceil(std::sqrt((double)M));
  ...
  public:
    ...
    std::vector<TreeNode<T> *> *search(const Bbox& bbox)
    {
      ...
      auto result = new std::vector<TreeNode<T> *>();      if (!intersects(bbox, node->bbox))
        return result;
